...
export default function Navbar() {
  ...
  return (
    <nav className="relative max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
      ...
      {/* ── Desktop nav ────────────────────────────────────────────────────────
          Breakpoint is lg, not md. Nine items at 11px plus padding need roughly
          875px of width; at md (768px) the tail of the list — Corporate,
          Personal, Contact — was being silently clipped by the page's
          `overflow-x: hidden` rather than wrapping or scrolling.
          ───────────────────────────────────────────────────────────────────── */}
      ...
      <button
        type="button"
        className="lg:hidden z-[100] flex flex-col gap-[5px] p-2"
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            transition={{ duration: reduceMotion ? 0.15 : 0.55, ease: [0.76, 0, 0.24, 1] }}
            className="fixed inset-0 bg-black z-[90] lg:hidden flex flex-col justify-between px-8 pt-28 pb-10 overflow-y-auto"
          >
              ...
              {NAV.map((link, i) => (
                <motion.div
                  ...
                  transition={
                    reduceMotion
                      ? { duration: 0 }
                      : { delay: 0.1 + i * 0.055, ease: [0.22, 1, 0.36, 1], duration: 0.5 }
                  }
            ...
            <p className="text-[10px] text-white/15 tracking-[0.25em] uppercase font-sans">
              &copy; {new Date().getFullYear()} PhotosByRH
            </p>
          </motion.div>
        )}
      </AnimatePresence>
