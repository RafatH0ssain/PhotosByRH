...
/* Warm the neighbouring photo into the HTTP cache. The quality must match the
   ...
   QUALITY must also appear in `images.qualities` in next.config.ts. Next
   answers an unlisted quality with a 400 rather than clamping it, and the
   failure is silent in the UI — the photo simply never arrives and the blur
   placeholder stays up forever. This was 85 against an allow-list of [75]. */
const QUALITY = 75;
      ...
      {isVisible && (
        ...
        >
          ...
          <div className="absolute top-6 left-7 z-10 flex items-baseline gap-[5px] pointer-events-none">
            <span className="font-mark text-[clamp(1.4rem,3.5vw,2.2rem)] leading-none tracking-[-0.02em] text-white">
              {counter}
            </span>
          ...
          >
            <span className="font-mark text-[10px] tracking-[0.22em] text-white/40 opacity-0 transition-opacity duration-200 group-hover:opacity-100 group-focus-visible:opacity-100">
              PREV
            </span>
          </button>
